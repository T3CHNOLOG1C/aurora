...
TextureHandle new_static_texture_2d(uint32_t width, uint32_t height, uint32_t mips, u32 format, ArrayRef<uint8_t> data,
  ...
  if (ref.gxFormat != InvalidTextureFormat) {
    ...
    /* OpenMelee TEMPORARY diagnostic (2026-08-18, fighter-texture
     ...
     * OPENMELEE_DUMP_TEX=1, output dir OPENMELEE_DUMP_TEX_DIR (default /tmp). */
    if (tlut && !converted.data.empty() && std::getenv("OPENMELEE_DUMP_TEX") != nullptr) {
      /* OpenMelee diagnostic: dump the decoded palette as a 1-row PPM so a
      ...
      const char* dir = std::getenv("OPENMELEE_DUMP_TEX_DIR");
